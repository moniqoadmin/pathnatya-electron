...
export async function syncTrustedTime(): Promise<number> {
  ...
  const latestVideoVersion = parseVideoVersion(data.latestVideoVersion)
  const videoVersionUpgraded = Boolean(
    currentVideoVersion &&
      latestVideoVersion &&
      isNewerVideoVersion(latestVideoVersion, currentVideoVersion)
  )

  if (videoVersionUpgraded) {
...
/** True when this process's latest time sync saw latestVideoVersion > currentVideoVersion. */
